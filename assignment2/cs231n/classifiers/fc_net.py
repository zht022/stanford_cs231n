...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # The first layer (between input_layer and the first hidden_layer):
        ...
        if self.use_batchnorm:
       	...
       	# The last layer (without batch normalization):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_batchnorm:
                ...
                layers[i+1], caches[i] = affine_bn_relu_forward(layers[i], W, b, gamma, beta, self.bn_params[i])
            else:
                layers[i+1], caches[i] = affine_relu_forward(layers[i], W, b)
            
            if self.use_dropout:
                layers[i+1], dp_caches[i] = dropout_forward(layers[i+1], self.dropout_param)
        	
        i = self.num_layers - 1
        W, b = self.params['W' + str(i+1)], self.params['b' + str(i+1)]
        layers[i+1] , caches[i] = affine_forward(layers[i], W, b)
